...
int triangulation::is_cross(edge s_in, edge s_out) // for edges
{
	...
	if (abs(t) <= eps && abs(t - ta1) <= eps && abs(t - tb1) <= eps) // edges on the same line
	{
		edge s;
		s.first.x = 0;
		s.first.y = 0;
	...
	if (abs(t) <= eps && abs(t - ta1) > eps && abs(t - tb1) > eps) return 0; //parallel edges
...
int triangulation::fillPointStore(vector<double>* x_ref, vector<double>* y_ref, vector<double>* f_ref)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		for (p_iter = pointStore.begin(); p_iter != pointStore.end(); p_iter++)
		{
			...
			if (abs(p.x - point_t.x) <= eps && abs(p.y - point_t.y) <= eps)
				k++;
		}
		if (k == 0)
		{
...
#pragma omp parallel for
                for (long long j = i; j < num_of_p; j++)
				{
					...
					if (abs(ed.first.x - ed.second.x) <= eps && abs(ed.first.y - ed.second.y) <= eps)
					{}
					else
        #pragma omp critical
...
{
    long long num = pointStore.size();
    long long k = 0, newK = 0;
    for (long long i = 0; i < num; i++)
    {
        fadeTr.insert(Point2(pointStore[i].x, pointStore[i].y, pointStore[i].f));
//        newK = (((double)i/num)*100);
//        if(newK > k)
//        {
//            k = newK;
...
//        }
    ...
    vector<Triangle2*> tmpTr;
    fadeTr.getTrianglePointers(tmpTr);
    for (long long i = 0; i < tmpTr.size(); i++)
    {
        triangle tmpTrian;
        tmpTrian.first.x = tmpTr[i]->getCorner(0)->x();   tmpTrian.third.x = tmpTr[i]->getCorner(2)->x();   tmpTrian.second.x = tmpTr[i]->getCorner(1)->x();
        tmpTrian.first.y = tmpTr[i]->getCorner(0)->y();   tmpTrian.third.y = tmpTr[i]->getCorner(2)->y();   tmpTrian.second.y = tmpTr[i]->getCorner(1)->y();
        tmpTrian.first.f = tmpTr[i]->getCorner(0)->z();   tmpTrian.third.f = tmpTr[i]->getCorner(2)->z();   tmpTrian.second.f = tmpTr[i]->getCorner(1)->z();
        triangleStore.push_back(tmpTrian);
    }
...
{
	if (abs(elem1.x - elem2.x) <= eps && abs(elem1.y - elem2.y) <= eps)
		return 1;
	else return 0;
}
...
int triangulation::corr_triangles(vector<triangle> *tri)
{
	...
	{
		if(abs(tr->first.x) <= eps && abs(tr->first.y) <= eps && abs(tr->second.x) <= eps && abs(tr->second.y) <= eps && abs(tr->third.x) <= eps && abs(tr->third.y) <= eps)
		{}
